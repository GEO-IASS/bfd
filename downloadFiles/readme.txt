...
bfdBound.m: Computes bound of marginal log-likelihood
bfd.m: Creates a model with specified parameters
bfdClassifyToyData.m: Function to classify TOY data sets
bfdClassifyData.m: Function used to classify a given dataset 
bfdCovarianceGradient.m: Gradient of L(\Theta) wrt Kernel matrix
bfdComputeAlpha.m: Computes coefficients alpha
bfdComputeL.m: Computes matrix L
bfdMatVar.m: Computes matrix associated to likelihood variance
bfdHist.m: Generates plot with histograms of projected data
bfdKernelGradient.m: Gradient of likelihood wrt kernel parameters.
bfdKernelObjective.m: Likelihood 
bfdOptimiseBFD.m: Adapts kernel parameters by EM algorithm
bfdPlot.m: Plots the discriminant function
bfdSaveData.m: Saves into a file the results of training a BFD model
bfdUpdateBeta.m: Update the value for beta.
bfdUpdateSigma.m: Update the posterior for Fisher's discriminant.
classSpecsRBFBIASWHITE.m: Script that generates vector of OPTIONS
classSpecsRBFARDLINARDBIASWHITE.m: Script that generates vector of OPTIONS 
computeKernel.m: Compute kernel given parameters LNTHETA and data X  
delElements.m: deletes columns or rows of a given matrix
demAUCbanana.m: Demo to obtain AUC statistics for banana dataset
demAUCbreastCancer.m: Demo to obtain AUC statistics for breast-cancer dataset
demAUCheart.m: Demo to obtain AUC statistics for heart dataset
demHistTwonorm.m: Demo for histograms of projections of Twonorm data  
demHistWaveform.m: Demo for histograms of projections of Waveform 
generateHist.m: Creates histogram of the output distribution
generateROC.m: Creates ROC curves for a given dataset
initTheta.m: Initialise the kernel parameters.
loadData.m: Loads data and labels for a given partition of a dataset
processROCdata.m: Computes mean and std from the ROC-generated data 
projectData.m: Projects training and test data over discriminant
selectParamsAndProject.m: projects data with selected model params 
selectParamsFromArray.m: selects median parameters 
setOptions.m: Converts an OPTSET structure into a vector of OPTIONS
thetaConstrain.m: Prevents kernel parameters from getting too big or small.
